...
@implementation RLStepper : CPControl
{
	CPButton	_upButton;
...
-(id)initWithFrame:(CGRect)aFrame
{
	...
	if(self){
		...
		var imagePath = function (inName) {
			...
			return [[CPBundle bundleForClass:[RLStepper class]] pathForResource:inName + @".png"];
		...
		[_upButton setImage:image("Stepper/stepper_up", CGSizeMake(12.0, 10.0))];
		[_bottomButton setImage:image("Stepper/stepper_down", CGSizeMake(12.0, 12.0))];
		
		[_upButton setAlternateImage:image(@"Stepper/stepper_up_highlighted", CGSizeMake(12.0, 10.0))];		
		[_bottomButton setAlternateImage:image("Stepper/stepper_down_highlighted", CGSizeMake(12.0, 12.0))];
		
		[_splitter setBackgroundColor:pattern(@"Stepper/stepper_middle", CGSizeMake(12.0, 1.0))];
